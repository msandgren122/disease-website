...
shinyUI(navbarPage(theme="orange_bootstrap.css", "Singapore Diseases",
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                p("This application makes simple time-series plots of various diseases in",
                                  ...
                                  "backspace."),
                                selectInput("disease_ts", "Disease(s):",
                                            c(colnames(dis)),
                                            selected=colnames(dis)[7],
                                            multiple = TRUE),
                                sliderInput("range_ts", "Adjust Time Range:",
                                            min=2006, max=2016,
                                            value=c(2006, 2016), sep="", dragRange = TRUE),
                                tags$hr(),
                                
                                selectInput("disease_season_ts", "Disease for seasonal plot:",
                                            c(colnames(dis)),
                                            selected=colnames(dis)[7],
                                            multiple = FALSE),
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                helpText("Note: NNAR Plots will take a few moments",
                                         "to render, as the model must be calculated."),
                                selectInput("disease_nn", "Disease:",
                                            c(colnames(dis)),
                                            selected=colnames(dis)[7],
                                            multiple = FALSE),
                                conditionalPanel(
